...
double analiza(Parameters parameters)
{
    ...
    if (parameters.ileObligacji != 1)
    {
        ...
        printf("Wszystkie wytworzone odsetki: %.2fzł\nKara za wcześniejszy wykup: %.2fzł\nZysk brutto: "
               ...
               result.wszystkieOdsetki * parameters.ileObligacji , result.kara * parameters.ileObligacji                    ,
               result.brutto * parameters.ileObligacji           , parameters.ileObligacji * (result.brutto - result.netto) ,
               result.netto * parameters.ileObligacji);
    }
...
int main(){
    ...
    const typObligacji types[] = {{.nazwa = "OTS", .kara = 99.0, .czas = 3,   .prc = 2.0,  .okresWyplaty = 0},
